---
title: "Reference Architecture of an Internal Developer Platform on AWS"
kind: Report
authors: "Weave Intelligence"
domain: "Cloud Native and Platform Engineering"
source_notion_url: "https://app.notion.com/3b8059b703e1816596fcc1cedfe2284e"
chapter_count: 1
capture_depth: full
...
Presents a layered reference architecture for delivering internal developer platform capabilities and golden paths on AWS.
### Questions this chapter answers
- Which layers make up an IDP reference architecture on AWS?
- How do developer-facing golden paths reach AWS resource planes?
- Where do security and observability controls apply?
### Key points
- The developer interface, platform control plane, and AWS resource plane have distinct responsibilities.
- Golden paths turn platform capabilities into repeatable application workflows.
- Security and observability are cross-cutting platform concerns.
...
flowchart LR
    ...
    C --> A["AWS resource plane"]
    A --> W["Application workloads"]
    S["Security and observability"] -. spans .-> C
    S -. spans .-> A
```
...
of an Internal Developer
Platform on AWS
WEAVE INTELLIGENCE PRESENTS
REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
PUBLISHED IN 2025
1    REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 2
Reference architecture of an Internal
Developer Platform on AWS
Table of contents
...
The Integration and Delivery Plane                          14
	...
	CD system                                               16
2       REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 3
Introduction
...
3   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 4
...
4   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 5
Overview of
changes in
version 2.0
...
5   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 6
...
6   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 7
Roles and responsibilities
	...
	ŗ Security platform engineers oversee the comprehensive security
		...
		automate security controls, establish secure-by-default configurations,
...
7   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 8
Design principles
	GitOps first
	...
	productive way.
8   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 9
...
9   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 10
Deep dive
...
10   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 11
PLATFORM TOOLING LANDSCAPE
	INTERNAL DEVELOPER PLATFORM ON AWS
11   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 12
...
12   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 13
Portals
...
13   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 14
Enterprises should also invest in developer education around prompt
	...
	This reference architecture uses Claude Code, an agentic
	coding assistant originally developed by Anthropic to accelerate
	software development by understanding your entire codebase
	and executing natural-language commands. Launched in early
	2025, it supports terminal, IDE and multi-file workflows and is
	rapidly gaining adoption across engineering teams.
...
14   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 15
Structuring your Version
	...
	reusable actions.
15   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 16
Image registry
...
16   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 17
The Resource Plane
	...
	on AWS and uses EKS for compute, RDS MySQL, Route 53 and
	Amazon SQS.
17   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 18
Security Plane
	...
	from the start.
	This reference architecture uses Sourcegraph, a code-
	intelligence platform that enables teams to search, understand
	and refactor large codebases by merging deep code-search, code
	navigation and AI-assistant capabilities. Founded in 2013, it
	has grown into a core tool for modern engineering orgs and
	now supports large-scale insights, batch changes and AI agents
	to industrialise software development.
	Secrets management
...
18   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 19
Policy control
	...
	This reference architecture uses SentinelOne Singularity
	Platform, an AI-powered cybersecurity platform that unifies
	endpoint protection, detection & response (EPP/EDR), cloud
	workload and identity security into a single solution. Founded
	in 2013 and publicly listed in 2021, it leverages behavioural
	machine learning to detect threats and automatically remediate
	compromised systems, reflecting its leadership in the modern
	cyber-defence landscape.
19   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 20
The Observability Plane
	...
	impact users.
20   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 21
This reference architecture uses Datadog, the most
	popular and widely used observability suite. Datadog is an
	observability platform that unifies metrics, traces, logs, and
	user-experience data across infrastructure and applications.
	Originally launched in 2010, and providing over 1,000
	built-in integrations covering cloud services, containers,
	...
	This reference architecture uses Kubecost, an open-source
	cost monitoring and optimization platform for Kubernetes
	environments. Developed by ex-Google engineers in 2019, it
	provides real-time visibility into cluster spending and resource
	efficiency by mapping usage of CPU, memory, GPU, and
	storage to actual cloud costs. By integrating with major cloud
	providers and FinOps tools, Kubecost enables platform teams
	to allocate costs accurately across namespaces, deployments, or
	teams, supporting effective showback, identifying expensive or
	overprovisioned workloads, and improving overall operational
	efficiency in multi-tenant Kubernetes clusters.
21   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 22
Incident Management
	...
	disruption to users.
22   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 23
Golden paths
...
23   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 24
Golden path 1: Adding an S3 bucket
to an existing workload
	A developer, aiming to enhance their application’s functionality, identifies
	the need for an S3 bucket to manage object storage. Leveraging the
	platform’s intuitive LLM-powered chat interface, they express their intent in
	natural language. The LLM, trained on the platform’s intricacies, seamlessly
	translates this request into a precise CLI command, which in turn initiates a
	GitOps workflow.
	...
	resource block under the relevant service, specifying the S3 bucket’s type
	and a desired name:
		# ... (previous content)
		resources:
			- type: s3
				name: my-data-bucket
	Upon approval and merge, the                   the provisioning of an S3 bucket
	change triggers the CI/CD pipeline.            and identifies the appropriate
	The pipeline orchestrates the build            infrastructure template to achieve
	process, compiling the application             this. However, before proceeding,
	code into a container image. This              the orchestrator consults the ID
	image then undergoes rigorous code             management system to verify
	analysis within the security plane,            that the developer possesses the
	scanning for potential vulnerabilities         necessary permissions to create
	and ensuring compliance with                   and manage S3 buckets.
	...
	The orchestrator, now aware                    members or security personnel.
	of the updated image and its                   If required, further security
	resource requirements, analyzes                checks are performed to assess
	the metadata and application                   the potential impact of the new
	context. It determines that the                resource on the overall system.
	new resource request necessitates              Upon successful completion of
24   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 25
these checks, the orchestrator                        S3 bucket to the developer.
	executes the Infrastructure as Code
	(IaC) defined within the chosen                       Simultaneously, the infrastructure
	template. This IaC interacts with the                 observability suite springs into
	cloud provider’s API, dynamically                     action, integrating the new S3
	provisioning the S3 bucket with the                   bucket into its monitoring and
	specified name and appropriate                        visualization framework. The
	configuration. The secret manager,                    bucket becomes a node within the
	integrated into the platform, securely                platform’s comprehensive resource
	retrieves the newly created bucket’s                  graph, allowing for real-time
	access credentials and injects them                   monitoring of its performance and
	as secrets into the application’s                     health. Additionally, the bucket’s
	container environment. This ensures                   endpoint is made accessible
	that the application can seamlessly                   through the platform’s portal,
	interact with the S3 bucket without                   providing the developer with a
	exposing sensitive information.                       convenient interface to manage its
	Finally, the orchestrator confirms the                contents and integrate it into their
	successful provisioning of the                        application’s workflow.
Golden path 2: Fleet updating
all S3 buckets in staging
...
01                        The IDE and                       The platform engineer initiates the process within
	their IDE, which is seamlessly integrated with the
	policy engine                     platform’s IaC tooling. As the engineer modifies the
STEP
	S3 bucket template, the policy engine, operating in
	the background, dynamically analyzes the changes.
	This real-time analysis ensures that the proposed
	modifications adhere to the organization’s security
	policies, compliance standards, and best practices.
	Any potential misconfigurations or violations are
	immediately flagged, preventing the introduction of
	errors or vulnerabilities into the infrastructure.
25     REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 26
02
	The platform,                     With the template changes validated, the engineer
		turns to the platform portal. This centralized dashboard
	portal, and                       provides a comprehensive view of the S3 bucket
STEP
	simulation                        landscape. The engineer can visualize all active buckets,
		their version history, and their deployment across various
		environments (development, staging, production).
		To mitigate risk, the engineer leverages the portal’s
		simulation capabilities. A simulation is executed,
		modeling the impact of the proposed changes on the
		existing environment variables and configurations. This
		predictive analysis helps identify potential conflicts or
		disruptions before the changes are applied.
03
	Platform                          The actual deployment of the S3 bucket updates is
		managed by the platform’s Platform Orchestrator. This
	orchestration                     system automates the rollout process, adhering to
STEP
	and progressive                   a progressive delivery strategy. Initially, the changes
	rollout                           are applied to a small subset of the development S3
		buckets. This allows for early testing and validation in
		a controlled environment. If no issues are detected,
...
04
	Observability                     Throughout the entire rollout process, the observability
		plane plays a crucial role. Metrics and logs from the S3
	and monitoring                    buckets are continuously collected and analyzed. The
STEP
	observability plane provides real-time insights into the
	health and performance of the updated infrastructure.
	Any anomalies or unexpected behaviors are immediately
	detected and alerted upon. The platform portal displays
	a live view of the rollout’s progress, including the updated
	S3 bucket versions across all environments. This
	transparency enables the engineer to track the impact of
	the changes and make informed decisions.
26     REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 27
Conclusion
...
27   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
### Page 28
...
28   REFERENCE ARCHITECTURE OF AN INTERNAL DEVELOPER PLATFORM ON AWS
## Code map
